...
fn assert_near(v: &Value, expected: f64) {
    let s = v
        .text()
        .unwrap_or_else(|| panic!("expected numeric bulk, got {v:?}"));
    let got: f64 = s
        .parse()
...
fn assert_coords(v: &Value, lon: f64, lat: f64) {
    let a = v
        .arr()
        .unwrap_or_else(|| panic!("expected [lon, lat], got {v:?}"));
...
fn assert_result(v: &Value, member: &str, fields: (Option<f64>, Option<&str>, Option<(f64, f64)>)) {
    let a = v
        .arr()
        .unwrap_or_else(|| panic!("expected record, got {v:?}"));
    let want = 1
...
fn geo_add_options() {
    ...
    // add 1 + CH + NX
    t.assert_int(
        &["geoadd", "Sicily", "CH", "NX", "14.25", "37.066667", "Gela"],
        1,
    );
...
fn geo_dist() {
    ...
    assert_near(
        &Value::Bulk(Some(
            d(&["geodist", "Sicily", "Palermo", "Catania"])
                .to_string()
                .into_bytes(),
        )),
        166_274.15156960033,
    ...
    assert_near(
        &Value::Bulk(Some(
            d(&["geodist", "Sicily", "Palermo", "Catania", "km"])
                .to_string()
                .into_bytes(),
        )),
        166.27415156960032,
    ...
    assert_near(
        &Value::Bulk(Some(
            d(&["geodist", "Sicily", "Palermo", "Catania", "MI"])
                .to_string()
                .into_bytes(),
        )),
        103.31822459492733,
    ...
    assert_near(
        &Value::Bulk(Some(
            d(&["geodist", "Sicily", "Palermo", "Catania", "FT"])
                .to_string()
                .into_bytes(),
        )),
        545_518.8699790037,
...
fn geo_search() {
    ...
    assert_result(
        ...
        "Berlin",
        (
            Some(0.00017343178521311378),
            Some("3673983950397063"),
    ...
    assert_result(
        ...
        "Dublin",
        (
            Some(487.5619030644293),
            Some("3678981558208417"),
            Some((6.2603, 53.3498)),
        ),
    );
    ...
    assert_result(
        &v[0],
        "Vienna",
        (Some(523.6926930553866), None, Some((16.3738, 48.2082))),
    );
    assert_result(
        &v[1],
        "Berlin",
        (Some(0.00017343178521311378), None, Some((13.4050, 52.5200))),
    );
    assert_result(
        &v[2],
        "Dublin",
        (Some(487.5619030644293), None, Some((6.2603, 53.3498))),
    );
    ...
    assert_result(
        &v[1],
        "Lisbon",
        (Some(502.20769462704106), None, Some((9.1427, 38.7369))),
    );
    ...
    assert_eq!(
        v.iter().map(Value::text).collect::<Vec<_>>(),
        vec![Some("Madrid".into()), Some("Lisbon".into())]
    );
}
...
fn geo_radius_by_member() {
    ...
    assert!(
        t.arr(&["GEORADIUSBYMEMBER", "invalid_key", "Madrid", "900", "KM"])
            .is_empty()
    );
    ...
    assert_result(
        &v[0],
        "Madrid",
        (Some(0.0), None, Some((3.703801, 40.416799))),
    );
    assert_result(
        &v[1],
        "Lisbon",
        (Some(502.207695), None, Some((9.142698, 38.736900))),
    );
    ...
    assert_eq!(
        ...
        vec![
            Some("Madrid".into()),
            Some("3471766229222696".into()),
            Some("Lisbon".into()),
            Some("3473121093062745".into())
        ]
    );
...
fn geo_radius_by_member_ub() {
    ...
    assert_result(
        ...
        (
            Some(0.0),
            None,
            Some((-122.41940170526505, 37.77490001056578)),
        ),
    );
}
